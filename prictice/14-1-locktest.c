...
main(void)
{
    ...
    // create a file
    if((fd=open("14-1-lockfile",O_RDWR|O_CREAT, 0666))<0)
    ...
    // Parents continues... read-lock the file again
    if((pid2=fork())<0){
        err_sys("fork filed");
    }else if(pid2==0){
        if(lock_reg(fd,F_SETLK, F_RDLCK,0,SEEK_SET,0)<0)
    ...
    // Parents continues... block while trying to write-lock
    if((pid3=fork())<0){
        err_sys("fork error.");
    }else if(0==pid3){
        ...
        printf("child 3: obout to block in write-lock...\n");
        if(lock_reg(fd,F_SETLK,F_WRLCK,0,SEEK_SET,0)<0){
            err_sys("child 3: can't write-lock file agin\n");
        }
        printf("child 3: returned and got write lock again ????\n");
        pause();
    ...
    if(lock_reg(fd,F_SETLK,F_RDLCK,0,SEEK_SET,0)<0){
        printf("Parents can't set read lock : %s\n", strerror(errno));
    }else{
        printf("Parents: obtained additional read lock while write lock is pending\n");
    }
